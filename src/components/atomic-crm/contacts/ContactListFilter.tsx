...
export const ContactListFilter = () => {
  ...
  return (
    <ResponsiveFilters searchInput={{ placeholder: translate("crm.search_name_company") }}>
      <FilterCategory label={translate("crm.last_activity")} icon={<Clock />}>
        <ToggleFilterButton
          className="w-auto md:w-full justify-between h-10 md:h-8"
          label={translate("crm.today")}
          value={{
        ...
        <ToggleFilterButton
          className="w-auto md:w-full justify-between h-10 md:h-8"
          label={translate("crm.before_this_week")}
          value={{
        ...
        <ToggleFilterButton
          className="w-auto md:w-full justify-between h-10 md:h-8"
          label={translate("crm.before_this_month")}
          value={{
            "last_seen@gte": undefined,
        ...
        <ToggleFilterButton
          className="w-auto md:w-full justify-between h-10 md:h-8"
          label={translate("crm.before_last_month")}
          value={{
            "last_seen@gte": undefined,
      ...
      <FilterCategory label={translate("crm.status")} icon={<TrendingUp />}>
      ...
      <FilterCategory label={translate("crm.tags")} icon={<Tag />}>
        {data &&
          data.map((record) => (
            <ToggleFilterButton
              className="w-auto md:w-full justify-between h-10 md:h-8"
      ...
      <FilterCategory icon={<CheckSquare />} label={translate("crm.tasks")}>
        <ToggleFilterButton
          className="w-full justify-between h-10 md:h-8"
          label={translate("crm.with_pending_tasks")}
          value={{ "nb_tasks@gt": 0 }}
          size={isMobile ? "lg" : undefined}
        />
      </FilterCategory>

      <FilterCategory icon={<Users />} label={translate("crm.account_manager")}>
        <ToggleFilterButton
          className="w-full justify-between h-10 md:h-8"
...
export const ContactListFilterSummary = () => {
  ...
  return (
    <div className="flex flex-wrap items-start mb-4 gap-1">
      ...
      <ActiveFilterButton
        className="w-auto justify-between h-8"
        label={translate("crm.managed_by_me")}
        value={{ sales_id: identity?.id }}
      />
    </div>
  );
